...
public class JwtUtil {
	// 비밀 키
	private static final String secretKey = "team3healthcare";
	private static final Logger logger = LoggerFactory.getLogger(JwtUtil.class);
	private static final Long TOKEN_VALID_TIME = 1000 * 60 * 60 * 12L; // 12시간 유효
	
	// JWT 생성 : 인증된 사용자 아이디만 포함
	public static String createToken(String staff_login_id) {
		String token = null;
		...
		try {
			...
			builder.claim("staff_login_id", staff_login_id);
	...
	public static String getEmail(String token) {
		...
		try {
			...
			email = claims.get("staff_login_id", String.class);
